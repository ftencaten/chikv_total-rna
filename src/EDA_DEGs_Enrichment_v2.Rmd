...
# Phenodata
pheno <- read_tsv(here("data", 'phenodata', "phenodata.tsv")) %>% 
...
%>% 
  deframe()
write_tsv(ranks_chronVSnonChron.D0,
          here("results", "DEG_edgeR", "ranks_chronVSnonChron.D0.txt"))

fgseaRes_chronVSnonChron.D0 <- fgsea(pathways=gene_set,
                                       stats=ranks_chronVSnonChron.D0, eps=0)

data.table::fwrite(fgseaRes_chronVSnonChron.D0, 
                   here("results", "GSEA", "fgseaRes_chronVSnonChron.D0.tsv"), 
                   sep="\t", sep2=c("", " ", ""))
