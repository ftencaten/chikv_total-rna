...
#tmm.csbl <- round(as.data.frame(cpm(csbl.expr)), digits = 1)
#tmm.csbl$Symbol <- csbl.expr$genes$gene_id

#write_tsv(tmm.csbl %>% relocate(Symbol), here("data", "chikv_csbl_star",
#                                              "tmm_toMDP.tsv"))
...
```{r}
arbo_edgeR.out <- read_tsv( here("results", "DEG_edgeR", 
                                 "arbobios_phase_condition.tsv"))

arbo_up <- arbo_edgeR.out %>% 
  filter(compar == "D0VSD21") %>% 
  filter(logFC >= 1, FDR < 0.1)

arbo_down <- arbo_edgeR.out %>% 
  filter(compar == "D0VSD21") %>% 
  filter(logFC <= -1, FDR < 0.1)

earlyVShealthy.up <- earlyVShealthy_t %>% 
   filter(logFC >= 1, FDR < 0.1)

earlyVShealthy.down <- earlyVShealthy_t %>% 
   filter(logFC <= -1, FDR < 0.1)

sum(arbo_down$GeneSymbol %in% earlyVShealthy.down$GeneSymbol)
...
# chronVSctrl.D0
earlyVShealthy_vp <- volc_plot(earlyVShealthy_t)
chronVSnonChron.early_vp <- volc_plot(chronVSnonChron.early_t)

patchw <- (earlyVShealthy_vp | chronVSnonChron.early_vp)

fp <- patchw + plot_annotation(tag_levels = 'A')

ggsave(here("results","figures","volcanoPlot_CSBL.png"), fp, 
       device = "png", width = 6, height = 4, units = "cm", 
       dpi = "print", scale = 2.5)
```
...
csbl_phase_chron_gsea <- fgseaRes_phase %>% 
  ...
  scale_x_discrete(name = NULL,
                   labels= c("Chron VS non-Chron - Early", "Early VS Healthy")) +
