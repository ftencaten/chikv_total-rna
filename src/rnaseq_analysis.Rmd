...
gene_set2 <- CEMiTool::read_gmt(here("data", "reactome_data", 
...
out.tmm.counts <- cbind(hannot[,1:2], tmm.counts)
...
#write_tsv(out.tmm,
#          here("data", "gene_expression", "counts_NOTmultimapping_TMM.tsv"))
...
## edgeR CAMERA
term.list <- split(gene_set2, as.factor(gene_set2$term))
path.list <- lapply(term.list, function(x) return(unname(as_vector(x[,2]))))

idx <- ids2indices(path.list, id = y$genes$GeneSymbol)

chronVSctrl.D0.enrich <- camera(y, idx, design, inter.gene.cor = 0.05,
                                contrast = my.contrast[, "chronVSctrl.D0"])

chronVSctrl.D21.enrich <- camera(y, idx, design, inter.gene.cor=0.05,
                                 contrast = my.contrast[, "chronVSctrl.D21"])

chronD21VSchronD0.enrich <- camera(y, idx, design, inter.gene.cor=0.05,
                                   contrast = my.contrast[, "chronD21VSchronD0"])

ctrlD21VSctrlD0.enrich <- camera(y, idx, design, inter.gene.cor=0.05,
                                  contrast = my.contrast[, "ctrlD21VSctrlD0"])

chronVSctrl.enrich <- camera(y, idx, design, inter.gene.cor=0.05,
                             contrast = my.contrast[, "chronVSctrl"])

D21VSD0.enrich <- camera(y, idx, design, inter.gene.cor=0.05,
...
## fgsea
pathways <- gmtPathways(here("data", "reactome_data", 
                             "ReactomePathwaysLevel3.gmt"))
...
fgseaRes_chronVSctrl.D0 <- fgsea(pathways=pathways, 
...
fgseaRes_chronVSctrl.D21 <- fgsea(pathways=pathways, 
...
fgseaRes_chronVSctrl <- fgsea(pathways=pathways, 
...
fgseaRes_chronD21VSchronD0 <- fgsea(pathways=pathways, 
...
fgseaRes_ctrlD21VSctrlD0 <- fgsea(pathways=pathways, 
...
fgseaRes_D21VSD0 <- fgsea(pathways=pathways, stats=ranks_D21VSD0, eps=0)
...
nes <- fgseaRes %>% 
  filter(if_any(starts_with('padj'), ~ . < 0.01)) %>%
...
corrplot::corrplot(as.matrix(t(nes)), p.mat = as.matrix(t(padj)), is.corr = F,
                   sig.level = .01, insig = "blank", tl.cex = 0.5, 
                   col = col3(100), tl.srt = 45)
