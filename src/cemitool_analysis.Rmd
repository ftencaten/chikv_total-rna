...
tmm_notmulti <- read_tsv(here("data", "gene_expression", 
                              "counts_NOTmultimapping_TMM.tsv"))
...
# Reactome TMM
tmm.counts <- as.data.frame(tmm_notmulti[!duplicated(tmm_notmulti$GeneSymbol),])
rownames(tmm.counts) <- tmm.counts$GeneSymbol

cem.reactome.tmm <- cemitool(expr = tmm.counts[,-c(1:2)], annot = smpl.annot, 
                         gmt = gmt.reactn, interactions = int_df, apply_vst = T, 
                         gsea_max_size = 2e3, verbose = TRUE)

generate_report(cem.reactome.tmm, title = "notMultimapping_Reactome_HGNC_TMM",
                directory = here("results", "CEMiTool", "Report",
                                 "notMultimapping_Reactome_HGNC_TMM"))
diagnostic_report(cem.reactome.tmm, title = "notMultimapping_Reactome_HGNC_TMM",
                  directory = here("results", "CEMiTool", "Diagnostic",
                                   "notMultimapping_Reactome_HGNC_TMM"))
